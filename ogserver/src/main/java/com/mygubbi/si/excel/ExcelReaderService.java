...
public class ExcelReaderService
{
    ...
    public List<KDMaxModule> mapKDMaxToMGModules()
    {
        ...
        List<KDMaxModule> kdMaxModules = new ArrayList<>();
        ...
        for (int i = 0; i < wb.getNumberOfSheets(); i++)
        {
            ...
            for (Row row : sheet)
            {
                ...
                KDMaxModule module = new KDMaxModule().setUnit(unit).setCode(moduleCode).setFinish(finish).setColor(color).setQuantity(quantity)
                        .setUom(uom).setRemarks(remarks).setName(name).setWidth(width).setDepth(depth).setHeight(height);
                kdMaxModules.add(module);
            }
        }
        try
        {
        ...
        for (KDMaxModule module : kdMaxModules)
        {
    ...
    {
        new ExcelReaderService("/testdata/Kitchen001-Quote.xlsx").mapKDMaxToMGModules();
        new ExcelReaderService("/testdata/Kitchen002-Quote.xlsx").mapKDMaxToMGModules();
        new ExcelReaderService("/testdata/Kitchen003-Quote.xlsx").mapKDMaxToMGModules();
        new ExcelReaderService("/testdata/Kitchen004-Quote.xlsx").mapKDMaxToMGModules();
    }
}
