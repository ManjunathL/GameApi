define([
    ...
    'models/filter'
], function($, jqueryui, _, Backbone, Bootstrap, productPageTemplate, productPageSmallGridTemplate, filterTemplate, Products, Categories, Filter) {
    var ProductPage = Backbone.View.extend({
            ...
            if(that.categories.isEmpty()){
                that.categories.fetch({
                    success: function() {
                        ...
                        that.filter.set({'selectedSubCategoriesList':selectedCategory.toJSON().subCategories},{silent: true});
                        that.filter.set({'minPriceLimit':selectedCategory.toJSON().minPriceLimit},{silent: true});
                        that.filter.set({'maxPriceLimit':selectedCategory.toJSON().maxPriceLimit},{silent: true});
                           ...
                           subCategory = that.categories.getSubCategoryBySubCategoryCode(selectedSubCategories);
                              ...
                              if(subCategory){
                                    ...
                                    that.filter.set({'selectedSubCategoriesName':subCategorynames},{silent: true});
                                    var subCategoryIds = new Array();
                                    subCategoryIds.push(subCategory.toJSON().id);
                                    that.filter.set({'subcategoryIds':subCategoryIds},{silent: true});
                              }
        ...
        //                    that.products.sortBy(that.model.sortBy, that.model.sortDir);
        //                    }
